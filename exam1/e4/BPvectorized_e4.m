% “Exam01, Problem 4 (Galapagos species regression)”
function BPvectorized_e4
% Prashant Kalvapalle
% COMP 502, Spring 2017,Exam 01 Problem IV
% Vectorized BP
...
numNodes = [5, 3, 1];  % set the number of nodes in each layers in the neural network including input layer - don't include bias nodes
weightMatrices = createWeightMatrices(numNodes,[0,1]);  % create the weight matrices for each hidden layer and output layer; [] = range of the weights

learningRate = 1e-2;
alpha = 0.05; % forgetting rate for momentum term >> Make it 0 for no momentum correction 

tanhSlope = 1;  % set the slope of the hyperbolic tangent function

maxLearnSteps = 1e5;  % number of times each batch is processed ; can terminate before if converged
errorTolerance = 0.001;  % scaled error tolerance (for inputs between [.1 - 1])

%% Input and Output samples for TRAINING the BP network
[trainInput, trainOutput, smthng] = read_gala();

maxTrainScale = max(max(trainOutput)); 
maxDiv = repmat(max(trainInput),size(trainInput,1),1);
scaledTrainInput = trainInput./maxDiv; % each row is a pattern; size 768 x 64
scaledTrainOutput = trainOutput/maxTrainScale; % output is same as input

%% Input and Output samples for TESTING the BP network

testInput = trainInput; testOutput = trainOutput;

maxTestScale =   max(max(testOutput));
scaledTestInput = testInput;
scaledTestOutput = testOutput/maxTestScale; % output is same as input
...
Erms_train = otherVariables{2}; % scaling already in the error function
% Erms_test = otherVariables{3}; 
...
disp(['<MSSE>:Lin.Reg = ',num2str(computeErrorMeasure(trainOutput,smthng,1))]) % its already in data scale
%% plot for Training accuracy 
figure;

plot(actualTrainOutput);
hold on
plot(trainOutput);
hold on
plot(smthng,'--')
...
legend('Learnt number','Actual number')
...
%         Erms_test(1,dum) = RMSE_test; Erms_test(2,dum) = i*k; % store errors and learning steps
        ...
        if RMSE_train < errorTolerance
            ...
            otherVariables{1} = total_steps; otherVariables{2} = Erms_train; otherVariables{3} = 0;%Erms_test; % giving output variables
            return
        end
    end
